...
def plot_workload_comparison(avx_csv, sse_csv, output_dir="plots"):
    ...
    for workload in workloads:
        ...
        display_name = (
            workload.replace("simple_vadd", "VecAdd").replace("_", " ").title()
        )

        # Convert SimTicks to numeric (handle 'N/A' values)
        merged["SimTicks_AVX"] = pd.to_numeric(
            merged["SimTicks_AVX"], errors="coerce"
        )
        merged["SimTicks_SSE"] = pd.to_numeric(
            merged["SimTicks_SSE"], errors="coerce"
        )
        ...
        if merged.empty:
            print(
                f"Warning: No valid numeric data for {workload}, skipping..."
            )
            continue
        ...
        ax1.set_ylabel(
            "Simulation Ticks [1e-7]", fontsize=12, fontweight="bold"
        )
        ax1.set_title(
            f"{display_name} - Performance Comparison",
            fontsize=14,
            fontweight="bold",
        )
        ax1.set_xticks(x)
        ...
        # Save bar chart
        output_file_bars = os.path.join(
            output_dir, f"{workload}_performance.png"
        )
        ...
        ax2.set_ylabel(
            "Speedup (AVX over SSE)", fontsize=12, fontweight="bold"
        )
        ax2.set_title(
            f"{display_name} - AVX Speedup", fontsize=14, fontweight="bold"
        )
        ...
        # Save speedup chart
        output_file_speedup = os.path.join(
            output_dir, f"{workload}_speedup.png"
        )
        ...
        print(
            f"    Min speedup: {merged['Speedup'].min():.2f}x (size {merged.loc[merged['Speedup'].idxmin(), 'Size']})"
        )
        print(
            f"    Max speedup: {merged['Speedup'].max():.2f}x (size {merged.loc[merged['Speedup'].idxmax(), 'Size']})"
        )
        print()
...
def create_summary_plot(avx_df, sse_df, workloads, output_dir):
    ...
    for workload in workloads:
        ...
        if not merged.empty:
            ...
            # Map workload names for display
            display_label = (
                workload.replace("simple_vadd", "VecAdd")
                .replace("_", " ")
                .title()
            )
    ...
    # Add reference lines
    ax.axhline(
        y=2.0, color="green", linestyle="--", alpha=0.5, label="2x baseline"
    )
    ax.axhline(
        y=2.5, color="orange", linestyle="--", alpha=0.5, label="2.5x target"
    )

    ax.set_ylabel(
        "Average Speedup (AVX vs SSE)", fontsize=12, fontweight="bold"
    )
    ax.set_title(
        "AVX-256 vs SSE-128 Performance Summary",
        fontsize=14,
        fontweight="bold",
    )
