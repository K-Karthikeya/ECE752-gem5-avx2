...
extern void avx_subset_test(double *a, double *b, double *out,
                            double *tmp, void *unused);
...
int main(void) {
    ...
    // Allocate 64 bytes to keep 32-byte alignment and avoid array-bounds
    // issues.
    ...
    if (tmp[2] != b[0] || tmp[3] != b[0] ||
        tmp[4] != b[2] || tmp[5] != b[2]) {
        fprintf(stderr, "MOVDDUP YMM: got={%.8f, %.8f, %.8f, %.8f}
                expect={%.8f, %.8f, %.8f, %.8f}\n",
